...
initial_hostile_objects = 1 # 초기 포식자 개체수
division_count = 4 # 분열 개체수
division_time = 450 # 분열 주기
division_percent = 50 # 분열 확률
max_passive = 5000 # 최대 피식자 개체수

safe_zone = 1 # 피식자 안전구역 (0:비활성화 /1:활성화)
linear = 200 # 안전구역 크기
...
def main():
    ...
    while count != initial_hostile_objects:
        ...
        if safe_zone != 1:
            ...
            while True:
                plag = 0
                point_x = random.randint(20, 1430)
                point_y = random.randint(80, 930)

                if (screen_width // 2) - (linear // 2) <= point_x <= (screen_width // 2) - (linear // 2) + linear:
                    if ((screen_height + 50) // 2) - (linear // 2) <= point_y <= ((screen_height + 50) // 2) - (linear // 2) + linear:
                        plag = 1

                if plag != 1:
                    hostile_mobs_list.append([[point_x, point_y], [100, 100], 1, 20, 0])
                    break
    ...
    while True:
                ...
                if event.key == ord('.'):
                    if safe_zone != 1:
                        ...
                        while True:
                            ...
                            if (screen_width // 2) - (linear // 2) <= point_x <= (screen_width // 2) - (
                                    linear // 2) + linear:
                                if ((screen_height + 50) // 2) - (linear // 2) <= point_y <= (
                                        (screen_height + 50) // 2) - (linear // 2) + linear:
                                    plag = 1
                    ...
                    del hostile_mobs_list[-1]

                if event.key == ord('m'):
                    gen_time = division_time
        ...
        if safe_zone == 1:
            pygame.draw.rect(screen, sky_blue, [(screen_width // 2) - (linear // 2),
                                                ((screen_height + 50) // 2) - (linear // 2), linear, linear], 0)
        ...
        text_2 = font_input_1.render("Object count : " + str(len(passive_mobs_list)) + " [" + str(len(hostile_mobs_list)) + "]", True, black)
        ...
        if fps != 0:
            ...
            for i in range(0, len(hostile_mobs_list)):
                ...
                if move >= hostile_mobs_list[i][2]:
                    ...
                    if before_direction == 1:
                        if safe_zone == 1:
                            before_point_x = hostile_mobs_list[i][0][0]
                            before_point_y = hostile_mobs_list[i][0][1]
                        direction = random.randint(1, 4)
                        if direction == 1: # 동
                    ...
                    if safe_zone == 1:
                        try:
                            safe_zone_range = pygame.Rect((screen_width // 2) - (linear // 2),
                                                          ((screen_height + 50) // 2) - (linear // 2), linear, linear)
                            hostile = pygame.Rect((hostile_mobs_list[i][0][0], hostile_mobs_list[i][0][1]),
                                                  (hostile_mobs_list[i][1][0], hostile_mobs_list[i][1][1]))
                            if safe_zone_range.colliderect(hostile):
                                hostile_mobs_list[i][0][0] = before_point_x
                                hostile_mobs_list[i][0][1] = before_point_y
                        except:
                            pass
